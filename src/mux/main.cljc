(ns mux.main
  "Kotodama WASM entrypoint for the Mux clean-room actor (L4) — Clojure port.
  ...
  facts (`mux.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Asset"      :plural "assets"      :id-prefix "mux_ass"
    :fields [:status :duration :resolutionTier :videoQuality :aspectRatio :test]
    :required [:status]
    :coerce {:duration :float :test :bool}
    :refs {}}
   {:entity "PlaybackId" :plural "playbackids" :id-prefix "mux_pla"
    :fields [:policy]
    :required [:policy]
    :coerce {}
    :refs {}}
   {:entity "Track"      :plural "tracks"      :id-prefix "mux_tra"
    :fields [:type :duration :maxWidth :maxHeight :status :primary]
    :required [:type]
    :coerce {:duration :float :maxWidth :int :maxHeight :int :primary :bool}
    :refs {}}
   {:entity "Upload"     :plural "uploads"     :id-prefix "mux_upl"
    :fields [:status :timeout :test]
    :required [:status]
    :coerce {:timeout :int :test :bool}
    :refs {}}
   {:entity "LiveStream" :plural "livestreams" :id-prefix "mux_liv"
    :fields [:status :latencyMode]
    :required [:status]
    :coerce {}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mux.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mux-compat" :tier tier :entities entities} 200])
